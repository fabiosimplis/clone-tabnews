...
describe("PATCH /api/v1/activations/[token_id]", () => {
  describe("Anonymous user", () => {
    test("With nonexistent token", async () => {
      const response = await fetch(
        "http://localhost:3000/api/v1/activations/fb576cff-bd47-4ecf-b9a0-a1e98060a22a",
        {
          method: "PATCH",
        },
      );
    ...
    test("With valid token", async () => {
      ...
      const activationUser = await user.findOneById(responseBody.user_id);
  ...
  describe("Default user", () => {
    test("With valid token, but already logged in user", async () => {
      ...
      const user2SessionToken = await activation.create(user2.id);

      const response = await fetch(
        `http://localhost:3000/api/v1/activations/${user2SessionToken.id}`,
        {
          method: "PATCH",
          headers: {
